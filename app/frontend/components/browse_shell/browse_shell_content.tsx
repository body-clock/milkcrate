...
function buildShellProps(opts: {
  ...
  return {
    mode, wall, currentCrates,
    activeSlug: opts.activeSlug, startIndex: opts.startIndex,
    selectCrate: opts.selectCrate,
    listingCount: opts.listingCount, genreCount: opts.genreCount,
    handleWallSelected, handleBrowseModeSelected,
  };
}

export default function BrowseShellContent({
  routing, activeSlug, startIndex, selectCrate, listingCount, genreCount, isWide,
}: Props) {
  const shellProps = buildShellProps({
    routing, activeSlug, startIndex, selectCrate, listingCount, genreCount,
  });
  if (isWide) {
