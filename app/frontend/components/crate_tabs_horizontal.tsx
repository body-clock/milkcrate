...
export default function HorizontalTabs({ crates, activeSlug, hasSelection, compact, tabsRef, activeTabRef, handleKeyDown, tabIndexValue, classesFn }: Props) {
  return (
    <div ref={tabsRef} role="tablist" aria-label="Crates" className={`flex overflow-x-auto ${compact ? "gap-1.5 scroll-px-1 px-1 pb-0.5" : "gap-1 pb-1"}`} style={{ scrollbarWidth: compact ? "none" : "thin" }}>
      {crates.map((crate, i) => (
        <TabButton key={crate.slug} crate={crate} selected={crate.slug === activeSlug} tabIndex={tabIndexValue(crate.slug === activeSlug, hasSelection, i)} activeTabRef={activeTabRef} onSelect={crate.slug} onKeyDown={(e) => handleKeyDown(e, i)} className={classesFn(compact, crate.slug === activeSlug)} />
      ))}
    </div>
  );
}
