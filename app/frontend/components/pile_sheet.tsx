...
function usePileSheetState(
  ...
  const { confirmClear, handleClose, handleClear, handleCancelClear, handleRequestClear } =
    usePileClearActions(onClose, clearPile);
  const { dialogRef, titleRef } = useDialogFocusTrap(open, handleClose, { returnFocusRef });
  return { pile, confirmClear, handleClose, handleClear, handleCancelClear,
    handleRequestClear, dialogRef, titleRef };
}
...
function useWantlistSender(
  ...
  return useCallback(async () => {
    ...
    await addToWantlist(pile.map((l) => ({ discogs_listing_id: l.discogs_listing_id })), storeSlug);
  }, [isConnected, storeSlug, pile, addToWantlist]);
}

function usePileSheet({
  open, onClose, returnFocusRef,
}: { open: boolean; onClose: () => void; returnFocusRef?: React.RefObject<HTMLElement | null> }) {
  const { pile, confirmClear, handleClose, handleClear, handleCancelClear, handleRequestClear,
    dialogRef, titleRef } = usePileSheetState(open, onClose, returnFocusRef);
  ...
  return { isCompact, dialogRef, titleRef, pile, confirmClear,
    handleClose, handleClear, handleCancelClear, handleRequestClear,
    storeSlug, handoffAvailable, storeName, shopper, isConnected,
    state, wantlistResult, errorMessage, resetResult, total, addToWantlist };
}
