...
export default function CrateChipBar({ title, description, crates, activeSlug, onSelectCrate }: Props) {
  if (crates.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-mc-border bg-mc-bg-card/70 px-4 py-5 text-sm text-mc-text-dim">
        No {title.toLowerCase()} crates yet.
      </div>
    );
  }

  const selectedSlug = activeSlug && crates.some((crate) => crate.slug === activeSlug) ? activeSlug : crates[0].slug;
    ...
    <div className="rounded-2xl border border-mc-border bg-mc-bg-card px-3 py-3 shadow-sm">
      <div className="mb-3 flex items-end justify-between gap-3 border-b border-mc-border pb-2">
        <div className="min-w-0">
          ...
          {description && <p className="mt-1 text-xs text-mc-text-dim leading-relaxed">{description}</p>}
        </div>
