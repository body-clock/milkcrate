...
const vertical = (
  s: ReturnType<typeof sharedState>,
  crates: Crate[],
  activeSlug: string | null,
  onSelect: (slug: string) => void,
) => (
  <VerticalTabs
    ...
    classesFn={verticalTabClasses}
  />
);

// eslint-disable-next-line react/no-multi-comp
// eslint-disable-next-line max-lines-per-function
function renderHorizontal(opts: {
  s: ReturnType<typeof sharedState>;
  crates: Crate[];
  activeSlug: string | null;
  compact: boolean;
  ...
  const { s, crates, activeSlug, compact, onSelect } = opts;
  return (
    <HorizontalTabs
      crates={crates}
      activeSlug={activeSlug}
      ...
      handleKeyDown={s.handleKeyDown}
      tabIndexValue={tabIndexValue}
      classesFn={horizontalTabClasses}
    />
  );
}
...
export default function CrateTabs({
  ...
  if (isVertical) {
    return vertical(s, crates, activeSlug, onSelect);
  }
  return renderHorizontal({ s, crates, activeSlug, compact, onSelect });
}
