...
function renderNavButton({ label, action, disabled, ariaLabel, isCompact }: {
  label: string; action: () => void; disabled: boolean; ariaLabel: string; isCompact: boolean;
}) {
  return (
    <motion.button type="button" onClick={action} disabled={disabled}
      whileTap={{ scale: SCALE_PRESS }} transition={springPress}
      className={navButtonClass(isCompact)} aria-label={ariaLabel}>
      {label}
    </motion.button>
  );
}
  ...
  return (
    <div className={`flex items-center justify-center ${isCompact ? "gap-3" : "gap-4 sm:gap-6"}`}>
      {renderNavButton({
        ...
        disabled: index <= 0,
        ariaLabel: RIFFLE_LANGUAGE.controls.front,
        isCompact,
      })}
      ...
      {renderNavButton({
        ...
        ariaLabel: RIFFLE_LANGUAGE.controls.deeper,
        isCompact,
      })}
    </div>
  );
