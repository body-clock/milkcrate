...
export function ToastMotion({ lastAdded, title, reducedMotion }: { lastAdded: { id: number }; title: string; reducedMotion: boolean }) {
  const anim = reducedMotion ? { opacity: 1 } : { opacity: 0, y: -8 };
  return (
    <motion.div key={lastAdded.id} role="status" aria-live="polite"
      initial={anim} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? { opacity: 0 } : { opacity: 0, y: -8 }}
