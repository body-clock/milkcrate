...
function riffleAnimY(direction: RiffleDirection, enter: boolean) {
  const positive = direction === "deeper";
  return enter === positive ? -ANIM_OFFSET : ANIM_OFFSET;
}
...
export default function RecordDetails({ listing, direction }: RecordDetailsProps) {
  ...
  return <AnimatePresence mode="wait" custom={direction}>
      <motion.div key={listing.id} custom={direction}
        initial={{ opacity: 0, y: riffleAnimY(direction, true) }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: riffleAnimY(direction, false) }} {...MOTION_PROPS}>
        <RecordMeta title={listing.title} artist={listing.artist} meta={meta} />
        <TagPills tags={buildTags(listing)} />
        {listing.notes && <p className="text-xs text-mc-text-dim leading-relaxed line-clamp-4">{listing.notes}</p>}
        <PriceAndActions listing={listing}
          inPile={inPile(listing.id)} onAdd={() => addToPile(listing)}
