...
const makeListing = (id: number): Listing => ({
  ...
  notes: null, discogs_url: `https://www.discogs.com/sell/item/${id}`,
});

const crate: Crate = {
  slug: "wall", name: "The Wall",
...
function renderWall(crateOverride?: Crate) {
  return renderWithTier("compact",
    <StorefrontMotionConfig><PileProvider><WallPanel crate={crateOverride ?? crate} /></PileProvider></StorefrontMotionConfig>,
  );
}
...
describe("WallPanel", () => {
  ...
  it("opens the peek sheet and returns focus to the originating tile", async () => {
    ...
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "Wall peek" })).not.toBeInTheDocument());
  });
});
