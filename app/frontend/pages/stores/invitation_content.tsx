...
function computeDisplayStatus(shouldProbe: boolean, slug: string, state: LookupState): "loading" | "found" | "not_found" {
...
export default function InvitationContent({ slug, oauth_available }: { slug: string; oauth_available?: boolean }) {
  const { state, lookup } = useDiscogsLookup();
  const csrfToken = getCsrfToken();
  const shouldProbe = useMemo(() => validateSlug(slug), [slug]);

  useEffect(() => { if (shouldProbe) {lookup(slug);} }, [slug, shouldProbe, lookup]);
  ...
  return (
    <MarketingLayout>
      <div className="flex flex-col items-center text-center pb-16 max-w-lg mx-auto">
        ...
        {displayStatus === "found" && <InvitationFound slug={slug} oauth_available={oauth_available} sellerName={sellerName} csrfToken={csrfToken} />}
