...
function renderErrors(fieldErrors: ErrorEntry[], copy: Copy) {
  return fieldErrors.flatMap(([field, errs]) => errs.map((e) => {
...
export default function ApplyErrors({ errorCount, fieldErrors, copy }: {
  errorCount: number;
  fieldErrors: ErrorEntry[];
  copy: Copy;
}) {
  if (errorCount === 0) {return null}
  const heading = errorCount === 1 ? "There's a problem with your submission." : `There are ${errorCount} problems with your submission.`;
  return (
    <FeedbackMessage tone="danger" live="assertive" className="px-4 py-3">
      <p className="font-semibold mb-1">{heading}</p>
      <ul className="list-disc list-inside text-xs text-mc-text-dim space-y-0.5">{renderErrors(fieldErrors, copy)}</ul>
    </FeedbackMessage>
  );
}
