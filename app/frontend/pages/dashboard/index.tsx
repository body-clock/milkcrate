...
export default function Dashboard({ store }: DashboardProps) {
  ...
  const syncStatusVariant = store.sync_status === "failed"
    ? "danger"
    : store.sync_status === "syncing" ? "working" : "neutral"
  ...
  const header = (
    <header className="mc-header flex items-center justify-between border-b border-mc-border px-4 py-3 sticky top-0 z-30 bg-mc-bg-raised/95 backdrop-blur-sm">
      <div className="flex items-center gap-3">
        <Link href="/" className="rounded text-xs text-mc-text-dim transition-colors hover:text-mc-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mc-focus">
          ← Home
        </Link>
            ...
            >
              <Card>
                <CardContent className="p-6 text-center">
                  <h2 className="text-lg font-bold text-mc-text mb-2">
                    Your store is live!
                  </h2>
