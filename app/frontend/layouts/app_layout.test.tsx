...
const mockedPage = vi.hoisted(() => ({
  props: {
    store: { name: "Philadelphia Music", discogs_username: "philadelphiamusic", handoff_available: true },
...
const listing: Listing = {
  id: 1, discogs_listing_id: "listing-1", artist: "Artist", title: "Title",
  label: null, year: null, format: null, genres: [], styles: [], condition: null,
  price: "10.00", currency: "USD", cover_image_url: null, thumbnail_url: null,
  notes: null, discogs_url: "https://www.discogs.com/sell/item/1",
};
...
describe("AppLayout wide", () => {
  beforeEach(reset);
  afterEach(() => { document.head.innerHTML = ""; });
  it("shows theme toggle", () => { renderWide(); expect(within(screen.getByRole("banner")).getByRole("button", { name: "Toggle light/dark mode" })).toBeInTheDocument(); });
  ...
  it("shows notice and alert", () => {
    mockedPage.props.notice = "Inventory updated"; const { unmount } = renderWide();
    expect(screen.getByRole("status")).toBeInTheDocument();
    unmount(); mockedPage.props.notice = undefined; mockedPage.props.alert = "Sync unavailable";
    renderWide(); expect(screen.getByRole("alert")).toBeInTheDocument();
  });
});
...
describe("AppLayout pile open", () => {
  ...
  it("sets inert on background when pile opens", async () => {
    const user = userEvent.setup(); renderCompact([listing]);
...
describe("AppLayout pile close", () => {
  ...
  it("returns focus to trigger after close", async () => {
    ...
    await user.click(trigger); await user.click(screen.getByRole("button", { name: "Close pile" }));
