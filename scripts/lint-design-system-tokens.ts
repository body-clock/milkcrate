...
const ARGV_INDEX = 2;
const root = process.argv[ARGV_INDEX] ?? "app/frontend";
...
for (const file of files) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const match of line.matchAll(deprecatedClassPattern)) {
      ...
      violations.push(
        `${rel}:${lineNumber}  deprecated class ${token}; use semantic utilities or canonical primitives`,
      );
    }

    for (const match of line.matchAll(rawStatusPalettePattern)) {
      violations.push(
        `${rel}:${lineNumber}  raw status palette ${match[0]}; use an mc-feedback semantic role`,
      );
    }

    for (const match of line.matchAll(obsoleteFocusPattern)) {
      violations.push(
        `${rel}:${lineNumber}  obsolete focus utility ${match[0]}; use focus-visible:ring-mc-focus`,
      );
    }
  }
}
