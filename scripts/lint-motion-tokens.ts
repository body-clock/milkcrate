...
for (const file of files) {
  ...
  if (file.endsWith("motion_tokens.ts") || file.endsWith("motion_tokens.test.ts")) {
    continue;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/stiffness:\s*\d+/.test(line)) {
      VIOLATIONS.push(
        `${rel}:${ln}  inline stiffness — use a spring preset from @/lib/motion_tokens`,
      );
    }
    if (/damping:\s*\d+/.test(line)) {
      VIOLATIONS.push(
        `${rel}:${ln}  inline damping — use a spring preset from @/lib/motion_tokens`,
      );
    }
    ...
    if (/whileHover.*scale:\s*\d/.test(line)) {
      VIOLATIONS.push(
        `${rel}:${ln}  whileHover with raw scale — use SCALE_HOVER from @/lib/motion_tokens`,
      );
    }
    if (/whileTap.*scale:\s*\d/.test(line)) {
      VIOLATIONS.push(
        `${rel}:${ln}  whileTap with raw scale — use SCALE_PRESS from @/lib/motion_tokens`,
      );
    }
    ...
    if (/transition.*type:\s*"spring".*(stiffness|damping)/.test(line)) {
      VIOLATIONS.push(
        `${rel}:${ln}  inline spring transition — use transitionHover/Drawer/Flip from @/lib/motion_tokens`,
      );
    }
  }
}
