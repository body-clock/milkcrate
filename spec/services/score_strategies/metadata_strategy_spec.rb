...
RSpec.describe ScoreStrategies::MetadataStrategy do
  ...
  describe "#score" do
    it "returns 0.0 for listings with styles, multiple genres, and year" do
      expect(strategy.score(build_listing(styles: [ "Modal" ], genres: [ "Jazz", "Fusion" ], year: 1975))).to eq(0.0)
    end

    it "returns -1.0 when listing has no styles, single genre, and no year" do
      expect(strategy.score(build_listing(styles: [], genres: [ "Jazz" ], year: nil))).to eq(-1.0)
    end

    it "returns 0.0 when listing has styles but no year or genre" do
      expect(strategy.score(build_listing(styles: [ "Modal" ], genres: [ "Jazz" ], year: nil))).to eq(0.0)
    end

    it "returns 0.0 when listing has year but no styles" do
      expect(strategy.score(build_listing(styles: [], genres: [ "Jazz" ], year: 1975))).to eq(0.0)
    end
