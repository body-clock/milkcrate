...
RSpec.describe RecordScorer do
  ...
  describe "#score" do
    ...
    it "ignores want/have ratio when have_count is below minimum threshold" do
      scorer      = build(:record_scorer, genre_counts: { "Jazz" => 5 }, today: Date.new(2026, 5, 5))
      thin_market = listing(want_count: 8, have_count: 3, genres: [ "Jazz" ])
    ...
    it "gives a mild bonus to records not surfaced in 14+ days" do
      scorer     = build(:record_scorer, genre_counts: { "Jazz" => 5 }, today: Date.new(2026, 5, 5))
      long_unseen = listing(id: 1, genres: [ "Jazz" ], last_surfaced_at: Date.new(2026, 4, 15))
      recently    = listing(id: 1, genres: [ "Jazz" ], last_surfaced_at: Date.new(2026, 4, 27))
    ...
    describe "cover quality" do
      it "boosts listings with a distinct cover image" do
        scorer     = build(:record_scorer, genre_counts: { "Jazz" => 5 }, today: Date.new(2026, 5, 5))
        good_cover = listing(id: 1, genres: [ "Jazz" ], cover_image_url: "https://example.com/cover.jpg", thumbnail_url: "https://example.com/thumb.jpg")
        thumb_only = listing(id: 1, genres: [ "Jazz" ], cover_image_url: "https://example.com/thumb.jpg", thumbnail_url: "https://example.com/thumb.jpg")
      ...
      it "penalizes listings where cover equals thumbnail" do
        scorer  = build(:record_scorer, genre_counts: { "Jazz" => 5 }, today: Date.new(2026, 5, 5))
        matched = listing(id: 1, genres: [ "Jazz" ], cover_image_url: "https://example.com/img.jpg", thumbnail_url: "https://example.com/img.jpg")
        none    = listing(id: 1, genres: [ "Jazz" ], cover_image_url: nil, thumbnail_url: nil)
      ...
      it "penalizes listings missing a cover image" do
        scorer    = build(:record_scorer, genre_counts: { "Jazz" => 5 }, today: Date.new(2026, 5, 5))
        thumb_only = listing(id: 1, genres: [ "Jazz" ], cover_image_url: nil, thumbnail_url: "https://example.com/thumb.jpg")
        none       = listing(id: 1, genres: [ "Jazz" ], cover_image_url: nil, thumbnail_url: nil)
